...
def solve(N, Nd, g, m, t, gr, dx, xx, yy, T_array, mu_array):
    ode15s.set_initial_value(u0, k_cutoff).set_f_params(N, Nd, g, mu_array[m],
                                                        T_array[t], gr, dx, xx,
                                                        yy)
    c = 1
    # input((k, len(k), N_k))
...
if __name__ == "__main__":
    ...
    for m in range(len(mu_array)):
        sol[m] = Parallel(n_jobs=num_cores)(delayed(solve)(
            N, Nd, g, m, t, gr, dx, xx, yy, T_array, mu_array) for t in
            range(len(T_array)))
    for m in range(len(mu_array)):
